...
int main(int argc, char *argv[])
{
	...
	char buffer[BUFLEN]; // storage messages
	char notify_others[100]; // mesaj de la server
	char aux[4];
	struct sockaddr_in serv_addr, cli_addr;
	int n, i, ret;
	socklen_t clilen;

	int clients[10]; // numarul maxim de clienti
	int clients_num = 0; // inceputul clientilor

	fd_set read_fds;	// multimea de citire folosita in select()
	fd_set tmp_fds;		// multime folosita temporar
	int fdmax;			// valoare maxima fd din multimea read_fds

	if (argc < 2) { 	// numarul de argumente
		usage(argv[0]);
	}
	...
	while (1) {
		...
		for (i = 0; i <= fdmax; i++) {
			if (FD_ISSET(i, &tmp_fds)) {
				...
				if (i == sockfd) {
					...
					// printf("New client %s, port %d, socket client %d\n",
					...
					if (n == 0) {
						...
						printf("Client <ID_CLIENT>%d disconnected.\n", i);
						close(i);

						sprintf(notify_others, "[UPDATE] Client %d disconnected\n", i);
		...
		fgets(ex, BUFLEN - 1, stdin);
        if (strncmp(ex, "exit", 4) == 0) return 0;
